...
public class FogOfWar : MonoBehaviour {
    ...
    void OnPreRender()
    {
        if (mWarFogMaterial == null || mViewers.Length == 0)
            return;

        for(int i = 0; i < mViewers.Length; i++)
        {
            Vector3 viewerWorldPos = mViewers[i].transform.position;
    ...
    void CreateProjectorMatrix()
    {
        ...
        vertices.Add(MapManager.Instance.TopLeft());
        vertices.Add(MapManager.Instance.BottomLeft());
        vertices.Add(MapManager.Instance.BottomRight());
        vertices.Add(MapManager.Instance.TopRight());
